...
import os
import random
from urllib.parse import quote_plus  # 🟢 Added for filename cleaning
...
async def channel_receive_handler(bot: Client, broadcast: Message):
    try:
        ...
        file_name = file.file_name if file else "Unknown_File"
        
        # 🟢 Clean the filename for the URL (e.g., 'Cars (2006).mkv' -> 'Cars+%282006%29.mkv')
        clean_name = quote_plus(file_name)

        msg = await broadcast.forward(chat_id=BIN_CHANNEL)
        
        # 🟢 Updated Link Structure: URL/ID/Clean_Name?hash=...
        raw_stream = f"{URL}watch/{msg.id}/{clean_name}?hash={get_hash(msg)}"
        raw_download = f"{URL}{msg.id}/{clean_name}?hash={get_hash(msg)}"
